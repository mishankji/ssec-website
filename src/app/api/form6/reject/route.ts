...
import { rejectForm6 } from "@/lib/google-sheets";
...
export async function POST(request: Request) {
  ...
  if (!isApprover(user.email)) {
    return NextResponse.json({ error: "Only approvers can reject submissions." }, { status: 403 });
  }
  ...
  try {
    const updated = await rejectForm6(body.submissionId.trim(), body.reason.trim());
    return NextResponse.json({ submission: toClientSubmission(updated) });
  } catch (err) {
    console.error("Form-6 rejection failed:", err);
